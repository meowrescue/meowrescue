...
const scriptToAdd = '<script src="/supabase-api-fix.js"></script>';
const insertAfter = '<script src="/supabase-fix.js"></script>';
...
function updateHtmlFile(filePath) {
  try {
    ...
    // Add our script after the supabase-fix.js script
...
// Main function
function main() {
  console.log('Starting Supabase fix application to all HTML files...');
  
  // Ensure the fix script exists
  const fixScriptPath = join(distDir, 'supabase-api-fix.js');
  if (!existsSync(fixScriptPath)) {
    console.error('Error: supabase-api-fix.js does not exist in the dist directory');
    return;
  }
  ...
  console.log('Complete! Applied Supabase fixes to all HTML files.');
}
